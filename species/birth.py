...
def derive_challenge(rappid_id: str, species: str) -> dict:
    """Deterministic from the creature AND its species, so a seal earned as one
    species can never be presented as another's. Same creature, same rite."""
    h = hashlib.sha256(f"{RITE}:{species}:{rappid_id}".encode()).digest()
